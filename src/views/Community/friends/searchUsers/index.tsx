...
const SearchUsers =()=> {
      ...
      const [friendRequestStatus, setFriendRequestStatus]=useState<{[id: string]: boolean }>({});
      useEffect(() => {
      ...
      return (
          <div className='search-users'>
            ...
            />
            {searchResults
                .filter((users) => users.some((user:any) => user.firstname || user.lastname))
                .map((users) => (
                  users.map((user: any) => (
                    <div className='add-friend' key={user.id}>
                      ...
                      {user.firstname || user.lastname ? (
                        <button onClick={() =>{addFriends(user.id) }}>
